...
public class ConnectDecoder extends BaseDecoder<ConnectMessage> {
    ...
    protected void decodeVariableHeader(ConnectMessage message, ByteBuf in) {
        ...
        message.setProtocolName(Utils.decodeString("Protocol Name", in));
    ...
    protected void decodePayload(ConnectMessage message, ByteBuf in) {
        // Payload
        // ClientID
        message.setClientID(Utils.decodeString("Client ID", in));

        // Will Topic and Will Message
        if (message.isWillFlag()) {
            message.setWillTopic(Utils.decodeString("Will Topic", in));
            message.setWillMessage(Utils.decodeString("Will Message", in));
        }

        // User Name and Password
        if (message.isUserNameFlag()) {
            message.setUserName(Utils.decodeString("User Name", in));

            if (message.isPasswordFlag()) {
                message.setPassword(Utils.decodeString("Password", in));
            }
        }
    }
}
